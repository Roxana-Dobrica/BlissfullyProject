...
function PatientActivities() {
  ...
  const renderActivities = (pageNumber) => {
    ...
    return activities.slice(startIndex, endIndex).map((activity, index) => (
      <div
  ...
  const updateActivityCompletionStatus = async (updatedActivity) => {
    try {
      const response = await fetch(
      ...
      if (!response.ok) {
        console.error("Failed to update activity completion status");
        return false;
      }
  ...
  const handleCheckboxChange = async (e, activity) => {
    e.stopPropagation();
    const updatedActivity = {
      ...activity,
      isActivityCompleted: e.target.checked,
    };

    setActivities((prevActivities) =>
      prevActivities.map((a) =>
        a.activityId === activity.activityId ? updatedActivity : a
      )
    );

    const success = await updateActivityCompletionStatus(updatedActivity);
    if (!success) {
      setActivities((prevActivities) =>
        prevActivities.map((a) =>
          a.activityId === activity.activityId
            ? { ...activity, isActivityCompleted: !e.target.checked }
            : a
        )
      );
      console.error("Failed to update activity status on server");
    }
  };
